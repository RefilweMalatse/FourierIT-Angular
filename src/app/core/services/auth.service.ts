...
import { AuditEventType, AuditLogEntry } from '../models/institution.models';
...
export class AuthService {
  ...
  private auditBase = `${environment.apiUrl}/audit`;
  ...
  login(payload: LoginPayload): Observable<AuthResponse> {
    return this.http.post<AuthResponse>(`${this.base}/login`, payload).pipe(
      tap(res => {
        ...
        this.logActivity(AuditEventType.LOGIN_SUCCESS, 'User signed in');
      })
    );
  }
  ...
  logout(): void {
    this.logActivity(AuditEventType.LOGOUT, 'User signed out');
  ...
  logActivity(actionType: AuditEventType, details?: string): void {
    const currentUser = this.currentUser();
    const entry: AuditLogEntry = {
      userId: currentUser?.id || currentUser?.email || 'anonymous',
      institutionId: currentUser?.id || 'unknown',
      timestamp: new Date().toISOString(),
      actionType,
      details,
    };

    this.http.post(this.auditBase, entry).subscribe({
  ...
  /**
   * Review documents is for Compliance Officers and Admins.
   */
  canReviewDocuments(): boolean {
    return this.hasRole('Compliance Officer') || this.hasRole('Admin');
  }
